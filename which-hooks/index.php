<?php
/*
Plugin Name: Which Hooks
Plugin URI: http://www.regency.global
Description: Which Hooks helps developers to quickly see where all the action and filter hooks are on any WordPress page.
Version: 1.0.1
Contributors: tapiwamavhunga, regencyglobal
Author: Tapiwa Mavhunga
Author URI: http://www.regency.global/?utm_medium=plugins%20page%20view%20details&utm_campaign=free%20plugin%20upsell&utm_source=send%20emails#plugins
License: GPLv2 or later
Text Domain: which-hooks
	...
	*/
	...
	function admin_bar_menu( $wp_admin_bar ) {
		...
		if ( 'show-action-hooks' == $this->status ) {
			...
			$title 	= __( 'Stop Showing Action Hooks' , 'which-hooks' );
			$href 	= add_query_arg( 'cxssh-hooks', 'off', $url );
			$css 	= 'cxssh-hooks-on cxssh-hooks-normal';
		}
		else {
			
			$title 	= __( 'Show Action Hooks' , 'which-hooks' );
			$href 	= add_query_arg( 'cxssh-hooks', 'show-action-hooks', $url );
			$css 	= '';
		}
		
		$wp_admin_bar->add_menu( array(
			'title'		=> '<span class="ab-icon"></span><span class="ab-label">' . __( 'Simply Show Hooks' , 'which-hooks' ) . '</span>',
			'id'		=> 'cxssh-main-menu',
			'parent'	=> false,
			'href'		=> $href,
		) );
		...
		if ( $this->status=="show-filter-hooks" ) {
			...
			$title	= __( 'Stop Showing Action & Filter Hooks' , 'which-hooks' );
			$href 	= add_query_arg( 'cxssh-hooks', 'off', $url );
			$css 	= 'cxssh-hooks-on cxssh-hooks-sidebar';
		}
		else {
			
			$title	= __( 'Show Action & Filter Hooks' , 'which-hooks' );
	...
	function notification_switch() {
		...
		<a class="cxssh-notification-switch" href="<?php echo esc_url( $url ); ?>">
			...
			<?php echo _e( 'Stop Showing Hooks' , 'which-hooks' ); ?>
		</a>
		<?php
	...
	public function load_translation() {
		load_plugin_textdomain( 'which-hooks', false, dirname( plugin_basename( __FILE__ ) ) . '/localization/' );
	}
					...
					<ul class="cxssh-hook-dropdown">
						...
						foreach ( $nested_hooks as $nested_key => $nested_value ) :
							...
							<li class="cxssh-priority">
								<span class="cxssh-priority-label"><strong><?php echo 'Priority:'; /* _e('Priority', 'which-hooks') */ ?></strong> <?php echo $nested_key ?></span>
							</li>
							<?php
